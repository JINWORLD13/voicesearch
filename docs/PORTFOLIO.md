...
검색 서비스. 신입 개발자 포트폴리오로 만들었다.

한 줄 요약: 음성 입력 → 웹 검색 → LLM이 근거 기반 답변을 스트리밍 → 음성 출력.
외부 API 키가 없어도 대체 경로로 동작하도록 설계했다.
...
처음엔 Exa 검색 API 하나만 썼다. 그런데 Exa는 가입과 API 키 발급이 필요해서,
이 앱을 처음 켜는 사람이 키 없이는 아무것도 못 보는 문제가 있었다.

Gemini에는 내장 구글 검색(grounding) 기능이 있다. 그래서 Exa 키가 없으면
Gemini가 스스로 검색해서 답하는 경로로 자동 전환하게 했다. 결과적으로
GEMINI_API_KEY 하나만 있으면 전체 기능이 돌아간다.
...
세밀한 제어가 필요하면 Exa, 간편함이 우선이면 내장 검색. 상황에 맞게 고르게 뒀다.

### 왜 LLM 프로바이더를 Gemini/OpenAI 둘 다 지원하나

특정 회사 API에 묶이지 않게 하려고 .env 값 하나(LLM_PROVIDER)로 바꾸게 했다.
두 SDK는 스트리밍 응답 모양이 다른데, 둘 다 "텍스트 조각을 내보내는
async generator"로 감싸서 라우트 코드는 어느 쪽인지 몰라도 되게 통일했다.

### 왜 SSE인가 (WebSocket 아니고)

답변 생성에 4~7초가 걸린다. 다 만들고 한 번에 주면 그동안 빈 화면이다.
서버가 조각을 만들어지는 대로 흘려보내면 글자가 차오른다.

방향이 서버→클라이언트 단방향뿐이라 SSE로 충분하다. SSE는 그냥 HTTP라
WebSocket보다 단순하다. 양방향(예: 생성 중단 신호)이 필요해지면 그때
WebSocket을 고려한다.

### 왜 음성은 폴백을 두었나

ElevenLabs 무료 플랜은 월 1만 자 제한이라 언제든 한도에 걸릴 수 있다.
음성 생성이 실패하면 브라우저 내장 음성(speechSynthesis)으로 대신 읽는다.
데모 중 API 한도 때문에 소리가 안 나는 상황을 없애기 위한 설계다.
...
먼저 해야만 검색을 써볼 수 있었다. 포트폴리오를 보는 사람 입장에선 키 하나
때문에 "검색 결과가 없습니다" 에러만 보게 된다.

원인 분석: 검색과 생성이 Exa에 강하게 묶여 있었다. 검색을 담당하는 부분만
갈아끼울 수 있으면 되는데 구조가 그렇지 못했다.

해결: 검색 라우트에서 키 유무로 경로를 나눴다. Exa 키가 있으면 기존대로,
없으면 Gemini 내장 구글 검색으로 검색과 생성을 한 번에 처리한다. 이미 갖고
있던 Gemini 키만으로 전체가 동작하게 됐고, 프론트는 이벤트 타입 기준으로
화면을 그리기 때문에 거의 손대지 않았다(delta에서 phase 전환 한 줄만 추가).

배운 것: 외부 의존성은 "없을 때의 경로"를 같이 설계해야 한다. 이게 음성
...
파싱이 터졌다. 재현이 잘 안 돼서 원인 찾기가 까다로웠다.

원인 분석: 네트워크는 내가 보낸 이벤트 단위로 데이터를 전달하지 않는다.
`data: {"type":"del` 까지만 도착하고 나머지 `ta","text":"..."}` 는 다음
조각에 오는 식으로, 이벤트 경계와 무관하게 잘려서 도착한다. 도착한 조각을
바로 파싱하니 반토막 JSON에서 에러가 난 것이다.
...
배운 것: 스트림은 메시지 단위가 아니라 바이트 단위로 온다. 경계를 직접
관리해야 한다는 걸 체감했다.
...
- 음성 인식이 Web Speech API 의존이라 크롬 계열에서만 동작. Whisper API로
  바꾸면 브라우저를 안 가리지만 녹음 업로드 구조가 필요하다.
- 단발 질문만 지원(멀티턴 없음).
- 음성 재생이 답변 생성 완료 후 시작. ElevenLabs 스트리밍 API로 문장 단위로
  미리 만들면 첫 소리까지의 대기를 줄일 수 있다.
- 배포는 Render(서버) + Netlify(프론트) 무료 티어로 예정.
