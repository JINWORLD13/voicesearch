...
方向がサーバー→クライアントの単方向だけなのでSSEで十分だ。SSEはただのHTTPなので
WebSocketより単純だ。双方向(例: 生成中断シグナル)が必要になったら、そのとき
WebSocketを検討する。
...
デモ中にAPI上限のせいで音が出ない状況をなくすための設計だ。
...
学んだこと: 外部依存は「ない場合の経路」を一緒に設計すべきだ。これは音声
...
そのままパースしたので、半分に千切れたJSONでエラーになったのだ。
...
- デプロイはRender(サーバー) + Netlify(フロント)の無料ティアで予定。
