...
読み上げてくれる検索サービスです。React、Express(TypeScript)、Gemini APIで
作りました。検索はExa APIまたはGemini内蔵のGoogle検索、音声合成は
ElevenLabsまたはブラウザ内蔵音声を使います。

## なぜ作ったのか

LLMは学習時点以降の出来事を知りません。以前、ドキュメントアップロード型のRAG
チャットボット(DocChat)を作りながら「根拠を与え、根拠の中だけで答えさせる」構造を
練習しましたが、今回はその根拠をアップロードされた文書ではなく、リアルタイムの
ウェブ検索から取得するように変えてみました。さらに入力は音声認識、出力は音声合成を
組み合わせて、画面を見続けなくても使える検索を目指しました。
...
回答は画面にも表示されますが、最終的には声で読み上げられます。そのためプロンプトで
マークダウン書式を禁止し、声に出して読んだとき自然な文章になるよう要求しました。
実際に「2.9%」の代わりに「2.9パーセント」のように読み下した回答が返ってきます。
出典番号 [1] は画面には必要ですが朗読すると不自然なので、TTSに送る前にサーバー側で
取り除きます。

### キーがなければ、あるもので動くように

外部キーが用意できていないせいでデモが止まるのが嫌だったので、有料・外部APIごとに
代替経路を用意しました。検索はExaキーがなければGemini内蔵のGoogle検索に、
音声合成はElevenLabsが失敗したら(キーなし・上限超過とも)ブラウザ内蔵の
speechSynthesisで代替します。そのため GEMINI_API_KEY 一つだけで全機能が
動きます。どちらの音声で再生中かは画面に表示し、ElevenLabs無料プラン
(月1万クレジット)を節約するため、TTSに送るテキストも600文字で切り詰めます。

### 出典を回答より先に送る

Exa経路では検索が終わると出典5件(タイトル、ドメイン、日付)を先に送信し、
その後に生成を始めます。生成を待つ間にも「何を根拠に答えるのか」が先に見えるので、
体感の待ち時間が短くなります。回答の文末には [1]、[2] でどの出典を使ったかを
示し、出典同士で内容が食い違う場合はより新しい日付を優先させています。
内蔵検索経路は検索と生成が1回の呼び出しなので順序が逆になりますが(回答ストリーミング後に
出典が到着)、フロントはイベントタイプだけを見て描画するため、両方の順序を処理できます。

### LLMプロバイダーの差し替え

GeminiとOpenAIを .env の LLM_PROVIDER の値一つで切り替えられます。両SDKの
ストリーミングAPIの形が違うので、どちらも「テキスト片を吐き出すasync generator」に
包んで統一しました。ルート側のコードはどのプロバイダーが繋がっているか知りません。
429、5xxのような一時的エラーは指数バックオフで再試行し、キーが間違っている401のような
エラーは再試行せず、すぐユーザーに原因を伝えます。

### 話し終えたら即検索

音声認識の中間結果(interim)を受け取り続けて入力欄にリアルタイム表示し、ブラウザが
発話終了と判断したら確定テキストで即座に検索を実行します。ボタンをもう一度押す
必要はありません。Web Speech APIはChrome系にしかないため、未対応ブラウザでは
マイクボタンを隠してテキスト検索だけを残します。

### 過負荷と障害に耐える4層防御

遅くて不安定な外部AIをラップするサーバーなので、「多く処理すること」より「外部が
揺れても耐えること」が核心だと考えました。Rate Limiter(Token Bucket自作) → Circuit
Breaker → Bulkhead(APIごとのセマフォ) → Graceful Degradation の4層に、キャッシュ・
再試行・タイムアウトを加えています。各決定の代替案比較と負荷テスト実測(キャッシュで
p50 3905→1ms、障害時にサーキットが壁時計時間を半減など)は
[docs/RESILIENCE.ja.md](docs/RESILIENCE.ja.md) にまとめました。ダッシュボードで
障害を注入しながら、防御が発動する様子をリアルタイムで観察できます。
...
質問してみてください。ELEVENLABS_API_KEYはなくても動作します(ブラウザ音声に
フォールバック)。

3. テスト (レジリエンスユーティリティ21個)
...
- 音声認識がWeb Speech API依存のためChrome系でしか動きません。WhisperのようなSTT
  APIに替えればブラウザを選びませんが、録音をサーバーに上げる構造が必要なので
  次の課題として残しました。
- 単発の質問のみ対応。前の質問を覚えるマルチターンはありません。
- 音声再生が回答生成の完了後に始まります。ElevenLabsのストリーミングAPIで文単位に
  先行生成すれば、最初の音が出るまでの待ちを減らせそうです。
- 内蔵検索経路は出典リンクがGoogleのリダイレクトURLで届き、検索結果の件数や
  本文抜粋を制御できず、回答内の [1] インライン番号も付けられません。
  細かい制御が必要ならExa経路を使えばよいです。
- デプロイはRender(サーバー) + Netlify(フロント)の無料ティア構成で公開予定です。
